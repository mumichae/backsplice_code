...
xy_train_human <- cbind(label = as.factor(y_train_human[,2]), data.frame(x_train_human))


# Fit the RF model
set.seed(72)

svm_model <- svm(label ~., data = xy_train_human, method = "C-classification", kernel= "radial", gamma = 0.1, cost = 10) # gamma and cost may have to be tuned
...
save(svm_model, file = "circRNA_SVM.RDS")
